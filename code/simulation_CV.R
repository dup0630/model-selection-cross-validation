...
hold_out <- function(alpha, X, y, ho_indices){
  n <- length(y)
...
k_fold_CV <- function(alpha, X, y, k=10){
  n <- length(y)
  partition <- split(1:n, sample(rep(1:k, length.out = n)))
  L_ho <- c()
  for (s in partition) {
    L_ho <- c(L_ho, hold_out(alpha, X, y, s)) # append new hold out loss
  }
  return(mean(L_ho))
}

# Correctness indicator
is_correct <- function(model, true_model){
  return(ifelse(length(model) >= length(true_model), 1, 0))
}

# Optimality indicator
is_best <- function(model, true_model){
  return(ifelse(length(model) == length(true_model), 1, 0))
}
...
# Leave-one-out
loo_MC <- function(r=1000, n, p, sigma_sq=1, true_model){
  correct <- 0
  best <- 0
  for (i in 1:r) {
    D <- generate_data(n, p, sigma_sq, true_model)
    X <- D[[2]]
    y <- D[[1]]
    values <- rep(0, p)
    for (j in 1:p) {
      alpha <- 1:(j+1)
      values[j] <- loo_CV(alpha, X, y)
    }
    selected_model <- 1:(which.min(values)+1)
    
    correct <- correct + is_correct(selected_model, true_model)
    best <- best + is_best(selected_model, true_model)
  }
...
# K-fold
k_fold_MC <- function(r=1000, k=10, n, p, sigma_sq=1, true_model){
  correct <- 0
  best <- 0
  for (i in 1:r) {
    D <- generate_data(n, p, sigma_sq, true_model)
    ...
    for (j in 1:p) {
      alpha <- 1:(j+1)
      values[j] <- k_fold_CV(alpha, X, y, k)
    }
    selected_model <- 1:(which.min(values)+1)
...
# Hold out
ho_MC <- function(r=1000, test_prop=0.2, n, p, sigma_sq=1, true_model){
  correct <- 0
  best <- 0
  ...
  for (i in 1:r) {
    ...
    for (j in 1:p) {
      ...
      values[j] <- hold_out(alpha, X, y, ho_split)
    }
    selected_model <- 1:(which.min(values)+1)
...
loo_time1 <- Sys.time()
loo_results <- loo_MC(r, n, p, sigma_sq, true_model)
loo_time2 <- Sys.time()
loo_time <- loo_time2 - loo_time1

kfold_time1 <- Sys.time()
kfold_results <- k_fold_MC(r, k=10, n, p, sigma_sq, true_model)
kfold_time2 <- Sys.time()
kfold_time <- kfold_time2 - kfold_time1

ho_time1 <- Sys.time()
ho_results <- ho_MC(r, test_prop = 0.5, n, p, sigma_sq, true_model)
ho_time2 <- Sys.time()
...
for (n in n_set) {
  loo_results[n] <- loo_MC(r, n, p, sigma_sq, true_model)
  kfold_results[n] <- k_fold_MC(r, k=10, n, p, sigma_sq, true_model)
  ho_results[n] <- ho_MC(r, test_prop = 0.5, n, p, sigma_sq, true_model)
}
