...
\begin{myproofbox}
    ...
    \begin{itemize}
        \item the function \(L(\lambda)\) from Definition~\ref{def:bic} is constant on each \(\Lambda_{m}\), \(0\leq m\leq M\), with\[L(\lambda) = 2\ln\paren{\frac{eM}{\min(M(\lambda), 1)}} =: L_{m}\]
