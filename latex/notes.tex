...
In an ideal scenario, we would like to select the weights \(\lambdahat{}\) that satisfy 
\[\lambda^{*} = \argmin_{\lambda\in\Lambda}\norm{f - \ftilde{lambda}}_{p}.\]
for some norm \(\norm{\cdot}_{p}\). However, since the true regression function \(f\) is unknown, this approach is clearly not feasible. Instead, we consider the \emph{oracle inequality} \begin{equation}
    \label{eq:oracleineq}
    \E\norm{f - \ftilde{\lambdahat{}}}_{p} \leq \inf_{\lambda\in\Lambda}\norm{f - \ftilde{\lambda}}_{p} + \Delta_{n,M}.
\end{equation}
for some error term \(\Delta_{n,M}>0\) independent of \(f\).
